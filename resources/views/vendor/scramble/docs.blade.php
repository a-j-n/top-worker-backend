...
<head>
    ...
    <script>
        const docsPasswordObserverRoots = new WeakSet();

        function revealDocsPasswordInputs(root) {
        ...
        function inspectDocsPasswordNode(node) {
            ...
            if (node instanceof HTMLInputElement && node.type === 'password') {
                node.type = 'text';
                node.setAttribute('data-password-visible', 'true');
            }

            if (node.shadowRoot) {
                observeDocsPasswordRoot(node.shadowRoot);
            }
            ...
            node.querySelectorAll('*').forEach((element) => {
                if (element.shadowRoot) {
                    observeDocsPasswordRoot(element.shadowRoot);
                }
            });
        }

        function observeDocsPasswordRoot(root) {
            if (! root || docsPasswordObserverRoots.has(root)) {
                return;
            }

            docsPasswordObserverRoots.add(root);
            revealDocsPasswordInputs(root);
                ...
                mutations.forEach((mutation) => {
                    if (mutation.type === 'attributes' && mutation.target instanceof HTMLInputElement) {
                        if (mutation.target.type === 'password') {
                            ...
                            mutation.target.setAttribute('data-password-visible', 'true');
                        }
                    ...
                    mutation.addedNodes.forEach((node) => {
                        inspectDocsPasswordNode(node);
                        revealDocsPasswordInputs(node);
                    });
                });
            });
        ...
        Element.prototype.attachShadow = function (options) {
            ...
            queueMicrotask(() => observeDocsPasswordRoot(shadowRoot));

            return shadowRoot;
        };

        document.addEventListener('DOMContentLoaded', () => {
            observeDocsPasswordRoot(document);
            observeDocsPasswordRoot(document.documentElement);
        });
    </script>
